...
- 分页和连续滚动共用居中的正文区域，宽屏下最大宽度为 840 dp；调整排版仍沿用正文字符位置恢复进度。
- 阅读排版面板提供即时预览和“恢复默认排版”，恢复操作保留主题、阅读方式、亮度及屏幕常亮偏好。
- 正文自带注释跟随阅读字号、字体和行距；目录行高随系统字号增长，较深层级限制缩进，给标题保留更多空间。
- 文内搜索面板重新打开时定位当前结果；目录定位计入顶部说明区域，避免滚动偏移。
- 阅读页和书架的批注编辑区支持滚动，方便小屏及大字号下编辑标签和批注。

## 本地精确笔记

0.4.0 起，长按正文可自由拖动选择字、词或字符；系统选择菜单支持直接复制文本，也可以选择“高亮/批注”保存用户笔记。同一段可以保存多条笔记，已有笔记会在正文中高亮；正文自带脚注仍作为只读内容注释单独展示。笔记使用段落指纹、字符范围和前后文进行重定位，设计说明见 [docs/NOTES.md](docs/NOTES.md)。该能力完全离线，不包含账号、同步、外部导入或文件导出。
